...
class AnalyzeCommand(BaseCommand):
    ...
    def _initialize_analysis(self, target_path: Optional[str]) -> None:
        ...
        if self.config.output.verbose:
            ...
            self._print_analysis_info(
                f"Excluding patterns: {self.config.analysis.exclude_patterns}"
            )
    ...
    def _create_progress_bar(self, description: str) -> Optional[Progress]:
        ...
        if (
            not self.config
            or not self.config.output.show_progress
            or self.config.output.format == "json"
        ):
            return None
    ...
    def _should_process_file(self, file_path: Path) -> bool:
        ...
        return not any(
            fnmatch(str(rel_path), pattern) for pattern in self.config.analysis.exclude_patterns
        )
    ...
    def _format_function_info(
        self, file_path: str, func_name: str, complexity: int, line: int
    ) -> str:
    ...
    def _print_summary(self, metrics: Dict[str, Any]) -> None:
        ...
        if complex_funcs:
            self.console.print(
                f"Functions with complexity >= {self.config.analysis.min_complexity}:"
            )
            for func in complex_funcs:
                self.console.print(
                    self._format_function_info(
                        func["file_path"], func["name"], func["complexity"], func["line"]
                    )
                )
